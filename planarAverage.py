...
def plot(periods=[], save=False):
    ...
    global x
    x = [i * latticelength[idir]/float(ngridpts[idir]-1)
    ...
    ax.set_xlabel('%s-axis ($\mathrm{\AA}$)' % direction)
...
#        sPeriod = Slider(axSlider, 'Period', 0.001, latticelength[idir],
#                         valinit=0.001, valstep=0.001, valfmt="%1.3f")
#        sPeriod.on_changed(update)
#        sPeriod.label.set_size(14)
    
    
    if save: plt.savefig(loc + '/macAvg', dpi=1000)
...
locs = [
    r'E:\Research\VASP Data\BiVO4-V2O5\slabs\BVO\001'
    ]
