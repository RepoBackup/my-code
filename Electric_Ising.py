...
def plot_cell(ax,bounds):
    ...
    points = []
    points += bounds_array
    vectors = [bounds_array[1] - bounds_array[0],
...
def draw_lattice(lat,P=np.array([]),ax=None,tilt=0,turn=0,aspect=1.85,
                 oxy=False,title=None,labels=False): 
...
#            continue
        ...
        if labels: ax.text(x+.25,y+.25,z+.25,i[1][2],fontsize=10) # add labels
        ...
        if i[0] not in struct.indices_from_symbol('O'):        
            ...
            ax.quiver(x, y, z, u, v, w, 
                      length=mag*2.5, color='k', 
                      arrow_length_ratio=0.1,
                      pivot='middle') # draw dipole moment
    
    if P.any():
        P = P / norm(P)
        ax.quiver(A/2, B/2, C/2, P[0], P[1], P[2], 
                  length=4., color='g', 
                  arrow_length_ratio=0.1,
...
def getCharges():
    ...
    c = []
    s = [line.split() for line in bader 
...
def build_lattice():
    ...
    lat = np.array([i for p in 
                    zip(coords, dipoles, labels) 
        ...
        def calcE():
            ...
            for j in range(L):
                if j != i:
                    ...
                    rn = norm(r)
                    rh = r / rn
                    E += 14.4/rn**3*(dot(p1,p2)-3*dot(p1,rh)*dot(p2,rh))
            return E
...
struct_no_O = struct.copy(); struct_no_O.remove_species('O')
